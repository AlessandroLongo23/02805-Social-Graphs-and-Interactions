...
def extract_genres_from_html(html_content):
    """
                            ...
                            if delimiter in genre_text:
                                for g in genre_text.split(delimiter):
                                    ...
                                    if g and not g.startswith('['):
                                        genres.append(normalize_genre(g))
                                break
                        else:
                            if genre_text and not genre_text.startswith('['):
                                genres.append(normalize_genre(genre_text))
...
def extract_genres_from_performers(performers_dir):
    """
        ...
        try:
            ...
            if 'parse' in data and 'title' in data['parse']:
                ...
                if 'text' in data['parse'] and '*' in data['parse']['text']:
                    ...
                    # Extract genres
                    genres = extract_genres_from_html(html_content)
...
def analyze_genres(artist_genres):
    ...
    plt.tight_layout()
    plt.savefig('Week 6/top_15_genres_histogram.png', dpi=300, bbox_inches='tight')
    print(f"\nHistogram saved to: Week 6/top_15_genres_histogram.png")
    plt.show()
    
    return genre_counts, top_15_genres

def save_artist_genres(artist_genres, output_file):
    """Save the artist-genre dictionary to a JSON file."""
    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(artist_genres, f, indent=2, ensure_ascii=False)
    print(f"Artist-genre dictionary saved to: {output_file}")

def main():
    # Path to performers directory
    performers_dir = "data/rock/performers"
    
    # Extract genres from all performers
    artist_genres = extract_genres_from_performers(performers_dir)
    
    if not artist_genres:
        print("No genres extracted. Please check the data.")
        return
    
    # Analyze and display statistics
    genre_counts, top_15_genres = analyze_genres(artist_genres)
    
    # Save results
    output_file = "Week 6/artist_genres.json"
    save_artist_genres(artist_genres, output_file)
    
    # Display a few examples
    print(f"\n{'='*60}")
    print(f"EXAMPLE ARTIST GENRES")
    print(f"{'='*60}")
