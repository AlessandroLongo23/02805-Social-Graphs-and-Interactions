...
def get_top_genres(artist_genres, n=15):
...
def load_performer_text(performers_dir, artist_name):
...
def clean_and_tokenize(text, remove_stopwords=True, lemmatize=False, min_word_length=3):
    """
...
def assign_artist_to_genres(artist, genres, method, top_genres):
    """
...
def aggregate_text_by_genre(artist_genres, performers_dir, top_genres, 
    ...
    for artist, genres in artist_genres.items():
        ...
        assigned_genres = assign_artist_to_genres(artist, genres, method, top_genres)
        
        if not assigned_genres:
            continue
        
        # Load and process text
        text = load_performer_text(performers_dir, artist)
...
def display_top_words(genre_tf, n=15):
...
def save_term_frequencies(genre_tf, output_file):
    ...
    print(f"\nTerm frequencies saved to: {output_file}")


def create_tf_summary(genre_tf, top_genres, method, output_file):
...
def main():
    print("="*70)
    print("TERM FREQUENCY (TF) ANALYSIS")
    print("="*70)
    ...
    top_genres = get_top_genres(artist_genres, n=n_top_genres)
    ...
    for method, method_name in methods:
        ...
        genre_tf = aggregate_text_by_genre(
            artist_genres, 
            performers_dir, 
            top_genres,
            method=method,
        ...
        # Display results
        display_top_words(genre_tf, n=15)
        
        # Save results
        output_dir = f"Week_7/tf_analysis_{method_name}"
        os.makedirs(output_dir, exist_ok=True)
        
        tf_file = f"{output_dir}/term_frequencies.json"
        save_term_frequencies(genre_tf, tf_file)
        
        summary_file = f"{output_dir}/tf_summary.png"
        create_tf_summary(genre_tf, top_genres, method_name, summary_file)
    ...
    print("\n" + "="*70)
    print("OBSERVATIONS AND NEXT STEPS")
    print("="*70)
    print("""
