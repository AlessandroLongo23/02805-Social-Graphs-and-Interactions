...
from tfidf_analysis import TFIDFAnalyzer, create_tfidf_wordcloud, create_tfidf_wordclouds
...
def extract_texts_from_performers(performers_dir='data/rock/performers'):
    """
...
def create_community_documents(artist_texts, partition, top_n=15):
    """
...
def compute_community_tf(community_docs, min_count=3):
    """
...
def create_combined_community_grid(community_tfidf, output_file, n_cols=3):
    """
...
""")
    ...
    output_dir = "Week_7/tfidf_communities"
    ...
    # Extract texts
    artist_texts = extract_texts_from_performers(performers_dir)
    ...
    community_docs = create_community_documents(artist_texts, partition, top_n=15)
    
    # Compute TF
    community_tf = compute_community_tf(community_docs, min_count=5)
    ...
    create_tfidf_wordclouds(community_tfidf, wordcloud_dir)
    
    # Create combined grid
    combined_file = os.path.join(wordcloud_dir, 'all_communities_tfidf_combined.png')
    create_combined_community_grid(community_tfidf, combined_file, n_cols=3)
