...
def load_term_frequencies(tf_file):
    """Load term frequency data from JSON file."""
...
def create_wordcloud(text, title, colormap='viridis', max_words=200, 
...
def plot_wordcloud(wordcloud, title, ax=None):
    """
    Plot a word cloud.
...
def save_individual_wordclouds(genre_tf, output_dir, colormap='viridis'):
    """
    ...
    for i, (genre, tf_dict) in enumerate(genre_tf.items(), 1):
        ...
        plot_wordcloud(wordcloud, f"{genre.title()} Genre", ax)
...
def create_combined_wordcloud_grid(genre_tf, output_file, n_cols=3):
    """
    ...
    for i, genre in enumerate(genres):
        ...
        # Plot
        plot_wordcloud(wordcloud, genre.title(), axes[i])
...
def analyze_wordcloud_results(genre_tf):
    """
...
def main():
    ...
    save_individual_wordclouds(genre_tf, wordcloud_dir, colormap='viridis')
    
    # Create combined grid
    combined_file = f"{wordcloud_dir}/all_genres_combined.png"
    create_combined_wordcloud_grid(genre_tf, combined_file, n_cols=3)
    
    # Analysis
    analyze_wordcloud_results(genre_tf)
